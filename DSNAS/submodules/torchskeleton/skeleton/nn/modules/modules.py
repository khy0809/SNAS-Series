...
class FactorizedReduce2d(torch.nn.Module):
    def __init__(self, stride=2):
        super(FactorizedReduce2d, self).__init__()
        self.stride = stride
    ...
    def forward(self, input):
        ...
        for y in range(self.stride):
            for x in range(self.stride):
                factorized.append(input[:, :, y::self.stride, x::self.stride])
        return torch.cat(factorized, dim=1)
